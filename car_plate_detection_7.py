...
smaller =cv2.resize(car_plate_1 , (550,740))
print(smaller.shape) #陣列是二+一維的 前兩維是xy 後一維是色彩RGB


gray = cv2.cvtColor(smaller, cv2.COLOR_BGR2GRAY)
print(gray.shape)#陣列是二維xy  imshow專拿來顯示RGB(三維的) 顯示二維的會出錯

arr[0,2].imshow(RGBgray(gray))  #我把二維的灰圖又擴增為三維灰圖 又加入了RGB
#灰圖
car_plate_2 =cv2.resize( cv2.imread("car.jpg", cv2.IMREAD_GRAYSCALE) , (550,740))
